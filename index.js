...
// require necessary modules for authentication
var basicAuth = require('express-basic-auth');
var sha256 = require('js-sha256');

// hashed base64 authentication
var pass = '901b281c4e0c4007e8526ef27153b79330811e733976d5e65c8343a39e54ec81';
...
//authorization function called by basicAuth
function authorizer(username, password){
    var hashed = sha256(username+':'+password);
    var matches = basicAuth.safeCompare(pass, hashed);
    return matches;
};
...
app.delete(apiPath + version + '/events/:eventId', basicAuth({authorizer: authorizer}), (req, res) => {
    if (!utility.isValidObjectID(req.params.eventId)) {
        return res.status(404).json({ "error": "Event not found!" });
    }
...
app.delete(apiPath + version + '/events/:eventId/bookings/:bookingId', basicAuth({authorizer: authorizer}), (req, res) => {
    if (!utility.isValidObjectID(req.params.eventId)) {
        return res.status(404).json({"message": "Event not found!"});
    }
